...
//				//brd의 detail임 => 수정된 내용을 보기 위해서, 업데이트된 내용
		...
		case "remove" :
			try {
				...
				log.info((isOk>0)? "파일삭제성공":"파일삭제실패");
				
				isOk = bsv.delete(bno);
				log.info("delete >>>>> "+((isOk>0)? "글삭제성공":"글삭제실패"));
				destPage = "pageList";
		...
		case "myList" :
			try {
				...
				PagingVO pgvo = new PagingVO(1,10,writer);
				
				if(request.getParameter("pageNo") != null) {
					int pageNo = Integer.parseInt(request.getParameter("pageNo"));
					int qty = Integer.parseInt(request.getParameter("qty"));
					log.info("pageNo : "+pageNo + ", qty : "+qty);
					pgvo = new PagingVO(pageNo, qty, writer);
				}

				int totalCount = bsv.myTotalCount(writer); 
				log.info("내 게시글 수 >>> " +totalCount);
				
				//bsv한테 pgvo주고 limit 적용한 리스트 가져오기
				List<BoardVO> list = bsv.myList(pgvo);
				request.setAttribute("list", list);
				
				//페이지 정보를 list.jsp로 보내기
				PagingHandler ph = new PagingHandler(totalCount, pgvo);
				request.setAttribute("ph", ph);
				log.info("myList >>> 페이징 성공!");
